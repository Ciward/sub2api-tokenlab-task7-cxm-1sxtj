...
```bash
GEMINI_OAUTH_CLIENT_ID=placeholder-client-id.example.invalid
GEMINI_OAUTH_CLIENT_SECRET=PLACEHOLDER_GOOGLE_CLIENT_SECRET

# 可选：如需使用 Gemini CLI 内置 OAuth Client（Code Assist / Google One）
# 安全说明：本仓库不会内置该 client_secret，请在运行环境通过环境变量注入。
# GEMINI_CLI_OAUTH_CLIENT_SECRET=PLACEHOLDER_BUILTIN_CLIENT_SECRET
```
...
2. Add your OAuth credentials in the `[Service]` section (after the existing `Environment=` lines):
   ```ini
   Environment=GEMINI_OAUTH_CLIENT_ID=placeholder-client-id.example.invalid
   Environment=GEMINI_OAUTH_CLIENT_SECRET=PLACEHOLDER_GOOGLE_CLIENT_SECRET
   ```

   如需使用“内置 Gemini CLI OAuth Client”（Code Assist / Google One），还需要注入：
   ```ini
   Environment=GEMINI_CLI_OAUTH_CLIENT_SECRET=PLACEHOLDER_BUILTIN_CLIENT_SECRET
   ```
