...
func TestResolveOrderPublicByResumeTokenReturnsRedactedSubscriptionSwitchInfo(t *testing.T) {
	gin.SetMode(gin.TestMode)
	t.Setenv("PAYMENT_RESUME_SIGNING_KEY", "0123456789abcdef0123456789abcdef")

	db, err := sql.Open("sqlite", "file:payment_handler_public_resolve_switch?mode=memory&cache=shared")
	require.NoError(t, err)
	t.Cleanup(func() { _ = db.Close() })
	...
	user, err := client.User.Create().
		SetEmail("public-switch@example.com").
		SetPasswordHash("hash").
		SetUsername("public-switch-user").
		Save(context.Background())
	require.NoError(t, err)
	...
	order, err := client.PaymentOrder.Create().
		...
		SetPayAmount(100).
		SetFeeRate(0).
		SetRechargeCode("PUBLIC-SWITCH").
		SetOutTradeNo("resolve-switch-order").
		SetPaymentType(payment.TypeAlipay).
		SetPaymentTradeNo("trade-public-switch").
	...
	_, err = client.PaymentAuditLog.Create().
		...
		SetAction(service.PaymentAuditActionSubscriptionKeySwitchResult).
		SetDetail(`{"action":"manual_switch_required","target_group_name":"Claude Pro","api_key_count":2,"auto_switched_key_name":"secret-key"}`).
		SetOperator("system").
		Save(context.Background())
	require.NoError(t, err)
	...
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, 0, resp.Code)
	switchData, ok := resp.Data["subscription_key_switch"].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "manual_switch_required", switchData["action"])
	require.Equal(t, "Claude Pro", switchData["target_group_name"])
	_, leakedCount := switchData["api_key_count"]
	_, leakedKeyName := switchData["auto_switched_key_name"]
	require.False(t, leakedCount)
	require.False(t, leakedKeyName)
}

func TestResolveOrderPublicByResumeTokenOmitsSwitchInfoBeforeCompletion(t *testing.T) {
	gin.SetMode(gin.TestMode)
	t.Setenv("PAYMENT_RESUME_SIGNING_KEY", "0123456789abcdef0123456789abcdef")

	db, err := sql.Open("sqlite", "file:payment_handler_public_resolve_switch_pending?mode=memory&cache=shared")
	require.NoError(t, err)
	t.Cleanup(func() { _ = db.Close() })
	...
	user, err := client.User.Create().
		SetEmail("public-switch-pending@example.com").
		SetPasswordHash("hash").
		SetUsername("public-switch-pending-user").
		Save(context.Background())
	require.NoError(t, err)
	...
	order, err := client.PaymentOrder.Create().
		...
		SetPayAmount(100).
		SetFeeRate(0).
		SetRechargeCode("PUBLIC-SWITCH-PENDING").
		SetOutTradeNo("resolve-switch-pending-order").
		SetPaymentType(payment.TypeAlipay).
		SetPaymentTradeNo("trade-public-switch-pending").
		SetOrderType(payment.OrderTypeSubscription).
	...
	_, err = client.PaymentAuditLog.Create().
		SetOrderID(strconv.FormatInt(order.ID, 10)).
		SetAction(service.PaymentAuditActionSubscriptionKeySwitchResult).
		SetDetail(`{"action":"manual_switch_required","target_group_name":"Claude Pro"}`).
		SetOperator("system").
		Save(context.Background())
	require.NoError(t, err)
	...
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, 0, resp.Code)
	_, hasSwitch := resp.Data["subscription_key_switch"]
	require.False(t, hasSwitch)
}
