...
// CustomMenuOpenMode specifies how a custom menu item opens its URL.
type CustomMenuOpenMode string

const (
	// CustomMenuOpenModeIframe opens the URL in an iframe within the app (default).
	CustomMenuOpenModeIframe CustomMenuOpenMode = "iframe"
	// CustomMenuOpenModeRedirect navigates directly to the URL (for sites that block iframe embedding).
	CustomMenuOpenModeRedirect CustomMenuOpenMode = "redirect"
)

// IsValidOpenMode checks if the given open mode is valid.
func IsValidOpenMode(mode string) bool {
	return mode == "" ||
		mode == string(CustomMenuOpenModeIframe) ||
		mode == string(CustomMenuOpenModeRedirect)
}
...
type CustomMenuItem struct {
	...
	OpenMode   CustomMenuOpenMode `json:"open_mode,omitempty"` // "iframe" (default) or "redirect"
}
