...
func (h *PaymentHandler) GetOrder(c *gin.Context) {
	...
	response.Success(c, buildPaymentOrderResponse(c.Request.Context(), h.paymentService, order))
}

// GetSubscriptionKeySwitchResult returns the post-subscription API key switch guidance for an order.
// GET /api/v1/payment/orders/:id/subscription-key-switch
func (h *PaymentHandler) GetSubscriptionKeySwitchResult(c *gin.Context) {
	subject, ok := requireAuth(c)
	if !ok {
		return
	}
	...
	result, err := h.paymentService.GetSubscriptionKeySwitchResult(c.Request.Context(), orderID, subject.UserID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
type PaymentOrderResult struct {
	*dbent.PaymentOrder
	SubscriptionKeySwitch *service.SubscriptionKeySwitchResult `json:"subscription_key_switch,omitempty"`
}

type PublicSubscriptionKeySwitchResult struct {
	Action          service.SubscriptionKeySwitchAction `json:"action"`
...
type PublicOrderResult struct {
	...
	SubscriptionKeySwitch *PublicSubscriptionKeySwitchResult `json:"subscription_key_switch,omitempty"`
}

func buildPublicOrderResult(ctx context.Context, paymentService *service.PaymentService, order *dbent.PaymentOrder) PublicOrderResult {
	var publicSwitch *PublicSubscriptionKeySwitchResult
	if persisted := paymentService.GetPersistedSubscriptionKeySwitchResultForOrder(ctx, order); persisted != nil && persisted.Action != service.SubscriptionKeySwitchActionNone {
		publicSwitch = &PublicSubscriptionKeySwitchResult{
			Action:          persisted.Action,
			TargetGroupName: persisted.TargetGroupName,
		}
	}
	return PublicOrderResult{
		...
		PlanID:                order.PlanID,
		SubscriptionKeySwitch: publicSwitch,
	}
}
...
func (h *PaymentHandler) VerifyOrderPublic(c *gin.Context) {
	...
	response.Success(c, buildPublicOrderResult(c.Request.Context(), h.paymentService, order))
}
...
func buildPaymentOrderResponse(ctx context.Context, paymentService *service.PaymentService, order *dbent.PaymentOrder) *PaymentOrderResult {
	if order == nil {
		return nil
	}
	return &PaymentOrderResult{
		PaymentOrder:          sanitizePaymentOrderForResponse(order),
		SubscriptionKeySwitch: paymentService.GetSubscriptionKeySwitchResultForOrder(ctx, order),
	}
}
