...
type RedeemService struct {
	...
	subscriptionKeySwitch *SubscriptionKeySwitchCoordinator
...
func NewRedeemService(
	...
	return &RedeemService{
		...
		userRepo:              userRepo,
		subscriptionService:   subscriptionService,
		subscriptionKeySwitch: subscriptionKeySwitch,
		affiliateService:      affiliateService,
		cache:                 cache,
		billingCacheService:   billingCacheService,
		entClient:             entClient,
...
func computeRedeemAffiliateBaseAmount(redeemType string, appliedAmount float64) float64 {
	if redeemType != RedeemTypeBalance {
		return 0
	}
	if appliedAmount <= 0 || math.IsNaN(appliedAmount) || math.IsInf(appliedAmount, 0) {
		return 0
	}
...
func (s *RedeemService) Redeem(ctx context.Context, userID int64, code string) (*RedeemCode, error) {
	...
	case RedeemTypeBalance:
		...
		affiliateBaseAmount = computeRedeemAffiliateBaseAmount(redeemCode.Type, amount)
	...
	if affiliateBaseAmount > 0 && s.affiliateService != nil {
		if _, err := s.affiliateService.AccrueInviteRebate(txCtx, userID, affiliateBaseAmount); err != nil {
	...
	if redeemCode.Type == RedeemTypeSubscription && redeemCode.GroupID != nil && redeemCode.ValidityDays >= 0 && s.subscriptionKeySwitch != nil {
		redeemCode.SubscriptionKeySwitch = s.subscriptionKeySwitch.HandleNewSubscription(ctx, userID, *redeemCode.GroupID)
	}
