...
// SyncAccountMetadataOnly refreshes non-token OpenAI account metadata using the existing access token only.
// It does NOT refresh token, rotate refresh_token, or call privacy mutation endpoints.
func (s *OpenAIOAuthService) SyncAccountMetadataOnly(ctx context.Context, account *Account) (*OpenAITokenInfo, error) {
	if account == nil {
		return nil, infraerrors.New(http.StatusBadRequest, "OPENAI_OAUTH_INVALID_ACCOUNT", "account is required")
	}
	if account.Platform != PlatformOpenAI {
		return nil, infraerrors.New(http.StatusBadRequest, "OPENAI_OAUTH_INVALID_ACCOUNT", "account is not an OpenAI account")
	}
	...
	if info := fetchChatGPTAccountInfo(ctx, s.privacyClientFactory, accessToken, proxyURL, orgID); info != nil {
		if info.PlanType != "" {
			tokenInfo.PlanType = info.PlanType
		}
		if info.SubscriptionExpiresAt != "" {
