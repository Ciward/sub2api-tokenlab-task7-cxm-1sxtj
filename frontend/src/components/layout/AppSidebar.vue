...
interface NavItem {
  ...
  expandOnly?: boolean
  /**
   * When set, clicking the item opens this external URL directly instead of
   * navigating to the internal `path`. Used for custom menu items with
   * open_mode='redirect' (sites that block iframe embedding).
   */
  externalLink?: string
  /**
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    ...customMenuItemsForUser.value.map((item): NavItem => ({
      ...
      // For direct redirect items: navigate to the external URL directly.
      // Keep the path for stable keying and fallback behavior (if user visits /custom/:id manually).
      externalLink: item.open_mode === 'redirect' ? item.url : undefined,
    })),
  )
  return items
}
  ...
  if (authStore.isSimpleMode) {
    ...
    for (const cm of customMenuItemsForAdmin.value) {
      filtered.push({
        ...
        icon: null,
        iconSvg: cm.icon_svg,
        externalLink: cm.open_mode === 'redirect' ? cm.url : undefined,
      })
    }
    return filtered
  }
