  ...
  >
    ...
    >
      ...
      <div v-if="form.platform === 'openai'">
        ...
        <div v-if="accountCategory === 'oauth-based'" class="mt-4 rounded-lg border border-gray-200 p-4 dark:border-dark-600">
          <label class="input-label">{{ t('admin.accounts.openai.templateLabel') }}</label>
          <select v-model="openAITemplateAccountId" class="input mt-2">
            <option :value="null">{{ t('admin.accounts.openai.templateNone') }}</option>
            <option v-for="opt in openAITemplateOptions" :key="opt.id" :value="opt.id">
              {{ opt.name }}{{ opt.status !== 'active' ? ` (${opt.status})` : '' }}
            </option>
          </select>
          <p class="input-hint">
            {{ openAITemplateLoading ? t('admin.accounts.openai.templateLoading') : t('admin.accounts.openai.templateHint') }}
          </p>
          <div v-if="selectedOpenAITemplate" class="mt-3 rounded-lg bg-blue-50 p-3 text-xs text-blue-700 dark:bg-blue-900/20 dark:text-blue-300">
            {{ t('admin.accounts.openai.templateUsing', { name: selectedOpenAITemplate.name }) }}
          </div>
        </div>
      </div>
      ...
      <template v-if="!usingOpenAITemplate">
...
interface OpenAITemplateOption {
  id: number
  name: string
...
const openAITemplateAccountId = ref<number | null>(null)
const openAITemplateOptions = ref<OpenAITemplateOption[]>([])
const openAITemplateLoading = ref(false)

const usingOpenAITemplate = computed(
  () => form.platform === 'openai' && accountCategory.value === 'oauth-based' && openAITemplateAccountId.value != null
)

const selectedOpenAITemplate = computed(() =>
  openAITemplateOptions.value.find((opt) => opt.id === openAITemplateAccountId.value) || null
)
...
const loadOpenAITemplateOptions = async () => {
  if (!(props.show && form.platform === 'openai' && accountCategory.value === 'oauth-based')) {
    return
  }
  openAITemplateLoading.value = true
  try {
    ...
    openAITemplateOptions.value = (resp.items || [])
      .filter((account: Account) => account.platform === 'openai' && account.type === 'oauth')
      .map((account: Account) => ({
        id: account.id,
        name: account.name,
        status: account.status,
    ...
    appStore.showError(t('admin.accounts.openai.templateLoadFailed'))
    openAITemplateOptions.value = []
  } finally {
    openAITemplateLoading.value = false
  }
}
...
watch(
  ...
  (newVal) => {
    if (newVal) {
      ...
      void loadOpenAITemplateOptions()
    } else {
      resetForm()
    }
  }
...
watch(
  [() => form.platform, accountCategory, () => props.show],
  ([platform, category, show]) => {
    if (show && platform === 'openai' && category === 'oauth-based') {
      void loadOpenAITemplateOptions()
    } else {
      openAITemplateAccountId.value = null
    }
  }
)
...
const handleGenerateUrl = async () => {
  if (form.platform === 'openai') {
    if (usingOpenAITemplate.value && openAITemplateAccountId.value != null) {
      await openaiOAuth.generateTemplateAuthUrl(openAITemplateAccountId.value)
    } else {
...
const handleOpenAIExchange = async (authCode: string) => {
  ...
  try {
    ...
    if (usingOpenAITemplate.value && openAITemplateAccountId.value != null) {
      const created = await oauthClient.createFromTemplateOAuth({
        templateAccountId: openAITemplateAccountId.value,
