<template>
  <div class="inline-flex flex-col gap-0.5 text-xs font-medium">
    ...
    <div v-if="expiresLabel" class="text-[10px] leading-tight text-gray-400 dark:text-gray-500 pl-0.5" :title="effectiveSubscriptionExpiresAt">
      {{ expiresLabel }}
    </div>
  </div>
</template>
...
const parseIdTokenExpiry = (idToken?: string): string => {
...
const effectiveSubscriptionExpiresAt = computed(() => props.subscriptionExpiresAt || parseIdTokenExpiry(props.idToken))

// Subscription expiration label (non-free only)
const expiresLabel = computed(() => {
  if (!effectiveSubscriptionExpiresAt.value || !props.planType) return ''
  if (props.planType.toLowerCase() === 'free') return ''
  try {
    const d = new Date(effectiveSubscriptionExpiresAt.value)
