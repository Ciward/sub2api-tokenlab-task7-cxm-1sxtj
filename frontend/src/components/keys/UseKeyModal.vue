...
const currentFiles = computed((): FileConfig[] => {
  ...
  switch (props.platform) {
    case 'openai':
      ...
      if (activeClientTab.value === 'openclaw') {
        return generateOpenClawFiles(apiBase, apiKey)
      }
  ...
  return [
    {
      ...
      content: buildCodexConfigToml(baseUrl),
...
function generateOpenAIWsFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  return [
    {
      path: `${configDir}/config.toml`,
      content: buildCodexConfigToml(baseUrl, { supportsWebsockets: true }),
      hint: t('keys.useKeyModal.openai.configTomlHint')
    },
    {
      path: `${configDir}/auth.json`,
...
function generateOpenClawFiles(baseUrl: string, apiKey: string): FileConfig[] {
  const isWindows = activeTab.value === 'cmd' || activeTab.value === 'powershell'
  const rootDir = isWindows ? '%userprofile%\\.openclaw' : '~/.openclaw'
  ...
  agents: {
    defaults: {
      model: { primary: "tokenlab/gpt-5.4" },
  ...
  return [
    { path: `${rootDir}/openclaw.json`, content: openclawConfig, hint: t('keys.useKeyModal.openclaw.configHint') }
  ]
}
