export default {
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      openclaw: {
        description: '直接在 openclaw.json 中配置 TokenLab 提供商即可，不再需要额外的 auth-profiles.json 或环境变量。',
        note: '现在推荐使用单文件 openclaw.json 方案。把下列配置合并到现有 openclaw.json 即可，里面已经包含 TokenLab provider、API Key 和默认模型示例。',
        noteWindows: '现在推荐使用单文件 openclaw.json 方案。把下列配置合并到现有 openclaw.json 即可，Windows 路径示例已按 %userprofile% 展示；不再需要额外的 auth-profiles.json 或环境变量。',
        envHint: '无需额外环境变量。',
        configHint: '这是完整的 openclaw.json 示例片段，直接合并进去即可。',
        authProfilesHint: '无需额外的 auth-profiles.json。'
      },
      antigravity: {
  ...
  subscriptionKeySwitch: {
    autoTitle: '已为你自动切换订阅分组',
    manualTitle: '请切换 API Key 到新订阅分组',
    noKeyTitle: '请创建或切换 API Key 到新订阅分组',
    autoDescription: '你刚获得了 {groupName} 订阅，系统已自动将你唯一的 API Key“{keyName}”切换到该订阅分组。',
    manualDescription: '你刚获得了 {groupName} 订阅，请前往 API Key 页面把要使用的 Key 切换到该订阅分组，否则请求可能继续消耗余额。',
    noKeyDescription: '你刚获得了 {groupName} 订阅，请前往 API Key 页面创建新的 Key，或将现有 Key 切换到该订阅分组，否则请求可能继续消耗余额。',
    goToKeys: '前往 API Keys',
    warning: '若未切到对应订阅分组，请求仍可能按照余额分组计费。',
    defaultGroupName: '新订阅分组',
    singleKeyFallbackName: '默认 API Key'
  },
  ...
  admin: {
    ...
    accounts: {
      ...
      openai: {
        ...
        templateLabel: '使用哪个模板',
        templateNone: '不使用模板（手动配置）',
        templateLoading: '正在加载可用模板账号…',
        templateHint: '选中模板后，将复制该账号的代理、分组、倍率、模型映射和 OpenAI 策略配置；下面的手动配置项会隐藏。',
        templateUsing: '当前将使用模板账号：{name}',
      },
      anthropic: {
    ...
    settings: {
      ...
      site: {
        ...
        subscriptionAutoSwitchSingleKey: '单 Key 用户新订阅自动切换分组',
        subscriptionAutoSwitchSingleKeyHint:
          '启用后，用户获取新订阅时若仅有一个 API Key，系统会自动把该 Key 切到对应订阅分组；否则只弹窗提醒用户手动切换。'
      },
      purchase: {
        title: '充值/订阅页面',
      ...
      customMenu: {
        title: '自定义菜单页面',
        description: '添加自定义页面到侧边栏导航。每个页面可以设置为普通用户或管理员可见。不支持 iframe 嵌入的外部网站可选择"直接跳转"方式。',
        itemLabel: '菜单项 #{n}',
        name: '菜单名称',
        ...
        openModeIframeHint: '在应用内通过 iframe 打开，适合支持嵌入的内部页面',
        openModeRedirect: '直接跳转',
        openModeRedirectHint: '在新窗口中直接打开，适合禁止 iframe 嵌入的第三方网站',
        add: '添加菜单项',
        remove: '删除',
        moveUp: '上移',
        moveDown: '下移',
  ...
  customPage: {
    ...
    redirectHint: '此页面配置为直接跳转模式，请点击下方按钮访问。',
