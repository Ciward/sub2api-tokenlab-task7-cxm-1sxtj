...
): Promise<{ auth_url: string; session_id: string; [key: string]: unknown }> {
  const { data } = await apiClient.post<{ auth_url: string; session_id: string; [key: string]: unknown }>(endpoint, config)
  return data
}
