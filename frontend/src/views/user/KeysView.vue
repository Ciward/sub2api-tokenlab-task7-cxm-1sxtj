...
import { buildCodexConfigToml } from '@/utils/codexConfig'
...
const resolvedApiBaseUrl = computed(() =>
  resolveApiBaseUrl(publicSettings.value?.api_base_url, window.location.origin)
)
...
const executeCcsImport = (row: ApiKey, clientType: 'claude' | 'gemini') => {
  const baseUrl = resolvedApiBaseUrl.value
  const platform = row.group?.platform || 'anthropic'
  ...
  if (platform === 'openai') {
    ...
    params.set('config', btoa(buildCodexConfigToml(endpoint)))
  } else {
