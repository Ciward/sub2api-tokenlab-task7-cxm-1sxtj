  ...
  <AppLayout>
    <div class="custom-page-layout">
      <div class="card flex-1 min-h-0 overflow-hidden">
        ...
        <!-- Direct redirect type: show link card instead of iframe -->
        <div v-else-if="isRedirectMode" class="flex h-full items-center justify-center p-10 text-center">
          <div class="max-w-md">
            <div
              class="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-gray-100 dark:bg-dark-700"
            >
            ...
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ menuItem?.label || t('customPage.title') }}
            </h3>
            <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">
              {{ t('customPage.redirectHint') }}
            </p>
            <a
              :href="menuItem?.url"
              target="_blank"
...
// Direct redirect mode: don't use iframe, show a link card instead.
// Empty/missing open_mode defaults to iframe (backward compatible).
const isRedirectMode = computed(() => {
  return menuItem.value?.open_mode === 'redirect'
})
