    ...
    <div class="w-full max-w-md space-y-6">
      ...
      <template v-else>
        ...
        <SubscriptionKeySwitchDialog
          :show="subscriptionKeySwitchDialogVisible"
          :result="subscriptionKeySwitchResult"
          @close="closeSubscriptionKeySwitchDialog"
          @go-keys="goToKeysAfterSubscription"
        />
      </template>
    </div>
  </div>
...
import SubscriptionKeySwitchDialog from '@/components/common/SubscriptionKeySwitchDialog.vue'
import {
...
import type { PaymentOrder, PaymentSubscriptionKeySwitchResult } from '@/types/payment'
...
const subscriptionKeySwitchDialogVisible = ref(false)
const subscriptionKeySwitchResult = ref<PaymentSubscriptionKeySwitchResult | null>(null)
const SUBSCRIPTION_KEY_SWITCH_NOTICE_PREFIX = 'subscription-key-switch-notice:'
...
function subscriptionKeySwitchNoticeKey(orderId: number): string {
  return `${SUBSCRIPTION_KEY_SWITCH_NOTICE_PREFIX}${orderId}`
}

function isSubscriptionKeySwitchNoticeDismissed(orderId: number): boolean {
  if (typeof window === 'undefined') return false
  return window.sessionStorage.getItem(subscriptionKeySwitchNoticeKey(orderId)) === '1'
}

function markSubscriptionKeySwitchNoticeDismissed(orderId: number): void {
  if (typeof window === 'undefined') return
  window.sessionStorage.setItem(subscriptionKeySwitchNoticeKey(orderId), '1')
}

function closeSubscriptionKeySwitchDialog(): void {
  if (order.value?.id) {
    markSubscriptionKeySwitchNoticeDismissed(order.value.id)
  }
  subscriptionKeySwitchDialogVisible.value = false
}

async function goToKeysAfterSubscription(): Promise<void> {
  closeSubscriptionKeySwitchDialog()
  await router.push('/keys')
}

function maybeShowSubscriptionKeySwitchResultFromOrder(): boolean {
  if (!order.value || !order.value.subscription_key_switch || order.value.subscription_key_switch.action === 'none') {
    return false
  }
  if (isSubscriptionKeySwitchNoticeDismissed(order.value.id)) {
    return false
  }
  subscriptionKeySwitchResult.value = order.value.subscription_key_switch
  subscriptionKeySwitchDialogVisible.value = true
  return true
}

async function maybeLoadSubscriptionKeySwitchResult(): Promise<void> {
  if (!order.value || order.value.order_type !== 'subscription' || !isSuccessStatus(order.value.status)) {
    return
  }
  if (isSubscriptionKeySwitchNoticeDismissed(order.value.id)) {
    return
  }
  if (maybeShowSubscriptionKeySwitchResultFromOrder()) {
    return
  }

  try {
    const response = await paymentAPI.getSubscriptionKeySwitchResult(order.value.id)
    const result = response.data
    if (!result || result.action === 'none') {
      return
    }
    subscriptionKeySwitchResult.value = result
    subscriptionKeySwitchDialogVisible.value = true
  } catch (_err: unknown) {
    // Ignore: this hint is best-effort and requires authenticated context.
  }
}
...
function scheduleStatusRefresh(refreshOrder: (() => Promise<PaymentOrder | null>) | null): void {
  ...
  statusRefreshTimer = setTimeout(async () => {
    ...
    if (refreshedOrder) {
      ...
      if (isSuccessStatus(refreshedOrder.status)) {
        void maybeLoadSubscriptionKeySwitchResult()
      }
    }
...
onMounted(async () => {
  ...
  if (isPendingStatus(order.value?.status)) {
    ...
    if (isSuccessStatus(order.value.status)) {
      await maybeLoadSubscriptionKeySwitchResult()
    }
