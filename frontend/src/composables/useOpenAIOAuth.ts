...
export interface OpenAITemplateSummary {
  id: number
  name: string
  platform: string
  type: string
...
export function useOpenAIOAuth() {
  ...
  const generateTemplateAuthUrl = async (
    templateAccountId: number,
    proxyId?: number | null,
    redirectUri?: string
  ): Promise<OpenAITemplateSummary | null> => {
    loading.value = true
    authUrl.value = ''
    sessionId.value = ''
    oauthState.value = ''
    ...
    try {
      ...
      const response = await adminAPI.accounts.generateAuthUrl(
        `${endpointPrefix}/template-auth-url`,
        payload
      )
      authUrl.value = response.auth_url
      sessionId.value = response.session_id
      ...
      return (response.template as OpenAITemplateSummary) || null
    } catch (err: any) {
      error.value = extractApiErrorMessage(err, t('admin.accounts.oauth.openai.failedToGenerateUrl'))
      appStore.showError(error.value)
      return null
  ...
  const createFromTemplateOAuth = async (params: {
    templateAccountId: number
    sessionId: string
    code?: string
    state?: string
    ...
    try {
      const payload: Record<string, unknown> = {
        template_account_id: params.templateAccountId,
      ...
      if (params.callbackUrl) {
        ...
        payload.state = params.state?.trim() || ''
      }
      if (params.redirectUri) {
        payload.redirect_uri = params.redirectUri
      }
      if (params.proxyId) {
        payload.proxy_id = params.proxyId
      }
      if (params.name?.trim()) {
      ...
      const account = await adminAPI.accounts.exchangeCode(
        `${endpointPrefix}/create-from-template-oauth`,
        payload
      )
      return account
    } catch (err: any) {
  ...
  return {
    ...
    resetState,
    generateAuthUrl,
    generateTemplateAuthUrl,
    exchangeAuthCode,
    createFromTemplateOAuth,
